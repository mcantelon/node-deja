...
exports.usage = function() {
  ...
  deja edit <REPO NAME><PATH/TO/FILE>\n\
  deja update\n\
  deja version"
}
...
exports.editRepoFile = function(dejaHome, repoFilePath) {
  ...
  path.exists(repoFilePathAbsolute, function(exists) {
    ...
    if (exists) {
      ...
      child_process.exec('mktemp', function(err, stdout, stderr) {
        ...
        var editor = (process.env.EDITOR) ? process.env.EDITOR : '/usr/bin/vim'

        var edit = spawn(editor, [repoFilePathAbsolute], {
          customFds: [
            process.stdin,
            process.stdout,
            process.stderr
...
exports.editCheat = function(dejaHome, cheat) {
  ...
  // get relative path from npm config, error if not set
  var getCheatPath = spawn('npm', ['get', 'dejaCheatPath'])
  ...
  getCheatPath.stdout.on('end', function() {
    ...
    if (cheatPath == 'undefined') {

      console.log('Error: issue the command "npm set dejaCheatPath <repo name[/path/to/cheats]>" to set the path to your cheats.')
    }
    else {
