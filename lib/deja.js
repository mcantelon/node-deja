        ...
        if (entry != '.git') {
          ...
          if (repo) {
            ...
            path.exists(pathToEntry, function(exists) {
              // if there's a corresponding entry in the home dir, display info
              if (exists) {
                fs.lstat(pathToEntry, function(err, stats) {
                  ...
                  entry_description += (attributes.length) ? ' [' : ''
                  entry_description += attributes.join(', ')
                  entry_description += (attributes.length) ? ']' : ''
                  console.log(entry_description)
                })
              }
              else {
              // no corresponding entry in home dir, display info
